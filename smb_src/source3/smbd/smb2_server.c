...
static void smbd_smb2_connection_handler(struct tevent_context *ev, struct tevent_fd *fde, uint16_t flags, void *private_data);
...
#define _OP(o) .opcode = o, .name = #o
    {
    _OP(SMB2_OP_NEGPROT), .as_root = true, },
    {
    _OP(SMB2_OP_SESSSETUP), .as_root = true, },
    {
    _OP(SMB2_OP_LOGOFF), .need_session = true, .as_root = true, },
    {
    _OP(SMB2_OP_TCON), .need_session = true,
    /*
    ...
    .as_root = true, },
    {
    _OP(SMB2_OP_TDIS), .need_session = true, .need_tcon = true, .as_root = true, },
    {
    _OP(SMB2_OP_CREATE), .need_session = true, .need_tcon = true, },
    {
    _OP(SMB2_OP_CLOSE), .need_session = true, .need_tcon = true, .fileid_ofs = 0x08, },
    {
    _OP(SMB2_OP_FLUSH), .need_session = true, .need_tcon = true, .fileid_ofs = 0x08, },
    {
    _OP(SMB2_OP_READ), .need_session = true, .need_tcon = true, .fileid_ofs = 0x10, },
    {
    _OP(SMB2_OP_WRITE), .need_session = true, .need_tcon = true, .fileid_ofs = 0x10, },
    {
    _OP(SMB2_OP_LOCK), .need_session = true, .need_tcon = true, .fileid_ofs = 0x08, },
    {
    _OP(SMB2_OP_IOCTL), .need_session = true, .need_tcon = true, .fileid_ofs = 0x08, .allow_invalid_fileid = true, },
    {
    _OP(SMB2_OP_CANCEL), .as_root = true, },
    {
    _OP(SMB2_OP_KEEPALIVE), .as_root = true, },
    {
    _OP(SMB2_OP_FIND), .need_session = true, .need_tcon = true, .fileid_ofs = 0x08, },
    {
    _OP(SMB2_OP_NOTIFY), .need_session = true, .need_tcon = true, .fileid_ofs = 0x08, },
    {
    _OP(SMB2_OP_GETINFO), .need_session = true, .need_tcon = true, .fileid_ofs = 0x18, },
    {
    _OP(SMB2_OP_SETINFO), .need_session = true, .need_tcon = true, .fileid_ofs = 0x10, },
    {
    _OP(SMB2_OP_BREAK), .need_session = true, .need_tcon = true,
    /*
     * we do not set
     * .fileid_ofs here
...
static void print_req_vectors(const struct smbd_smb2_request *req)
{
	...
	{
		dbgtext("\treq->in.vector[%u].iov_len = %u\n", (unsigned int) i, (unsigned int) req->in.vector[i].iov_len);
	}
	for (i = 0; i < req->out.vector_count; i++)
	{
		dbgtext("\treq->out.vector[%u].iov_len = %u\n", (unsigned int) i, (unsigned int) req->out.vector[i].iov_len);
	}
}
...
static NTSTATUS smbd_initialize_smb2(struct smbXsrv_connection *xconn)
{
	...
	xconn->transport.fde = tevent_add_fd(xconn->ev_ctx, xconn, xconn->transport.sock, TEVENT_FD_READ, smbd_smb2_connection_handler, xconn);
...
static NTSTATUS smbd_smb2_inbuf_parse_compound(struct smbXsrv_connection *xconn, NTTIME now, uint8_t *buf, size_t buflen, struct smbd_smb2_request *req, struct iovec **piov, int *pnum_iov)
{
	...
	while (taken < buflen)
	{
		...
		if (IVAL(hdr, 0) == SMB2_TF_MAGIC)
		{
			...
			{
				DEBUG(10, ("Got SMB2_TRANSFORM header, " "but dialect[0x%04X] is used\n", xconn->smb2.server.dialect));
				goto inval;
			}

			if (!(xconn->smb2.server.capabilities & SMB2_CAP_ENCRYPTION))
			{
				DEBUG(10, ("Got SMB2_TRANSFORM header, " "but not negotiated " "client[0x%08X] server[0x%08X]\n", xconn->smb2.client.capabilities, xconn->smb2.server.capabilities));
				goto inval;
			}
			...
			{
				DEBUG(1, ("%d bytes left, expected at least %d\n", (int)len, (int)(SMB2_TF_HDR_SIZE + enc_len)));
				goto inval;
			}
			...
			if (s == NULL)
			{
				DEBUG(1, ("invalid session[%llu] in " "SMB2_TRANSFORM header\n", (unsigned long long)uid));
			...
			status = smb2_signing_decrypt_pdu(s->global->decryption_key, xconn->protocol, tf_iov, 2);
		...
		if (num_iov >= ARRAY_SIZE(req->in._vector))
		{
			...
			iov_tmp = talloc_realloc(mem_ctx, iov_alloc, struct iovec, num_iov + SMBD_SMB2_NUM_IOV_PER_REQ);
			if (iov_tmp == NULL)
			{
				TALLOC_FREE(iov_alloc);
				return NT_STATUS_NO_MEMORY;
			...
			if (iov_alloc == NULL)
			{
				memcpy(iov_tmp, req->in._vector, sizeof(req->in._vector));
			}
...
static NTSTATUS smbd_smb2_request_create(struct smbXsrv_connection *xconn, const uint8_t *_inpdu, size_t size, struct smbd_smb2_request **_req)
{
	...
	status = smbd_smb2_inbuf_parse_compound(xconn, now, inpdu, size, req, &req->in.vector, &req->in.vector_count);
	if (!NT_STATUS_IS_OK(status))
	{
		TALLOC_FREE(req);
		return status;
...
static bool smb2_validate_sequence_number(struct smbXsrv_connection *xconn, uint64_t message_id, uint64_t seq_id)
{
	...
	{
		DEBUG(0,
		    ("smb2_validate_sequence_number: bad message_id " "%llu (sequence id %llu) " "(granted = %u, low = %llu, range = %u)\n", (unsigned long long)message_id, (unsigned long long)seq_id, (unsigned int)xconn->smb2.credits.granted, (unsigned long long)xconn->smb2.credits.seq_low, (unsigned int)xconn->smb2.credits.seq_range));
		return false;
	}
	...
	{
		DEBUG(0,
		    ("smb2_validate_sequence_number: duplicate message_id " "%llu (sequence id %llu) " "(granted = %u, low = %llu, range = %u) " "(bm offset %u)\n", (unsigned long long)message_id, (unsigned long long)seq_id, (unsigned int)xconn->smb2.credits.granted, (unsigned long long)xconn->smb2.credits.seq_low, (unsigned int)xconn->smb2.credits.seq_range, offset));
		return false;
	}
	...
	{
		DEBUG(10, ("smb2_validate_sequence_number: clearing " "id %llu (position %u) from bitmap\n", (unsigned long long)(xconn->smb2.credits.seq_low), offset));
...
static bool smb2_validate_message_id(struct smbXsrv_connection *xconn, const uint8_t *inhdr)
{
	...
	DEBUG(11,
	    ("smb2_validate_message_id: mid %llu (charge %llu), " "credits_granted %llu, " "seqnum low/range: %llu/%llu\n", (unsigned long long) message_id, (unsigned long long) credit_charge, (unsigned long long) xconn->smb2.credits.granted, (unsigned long long) xconn->smb2.credits.seq_low, (unsigned long long) xconn->smb2.credits.seq_range));

	if (xconn->smb2.credits.granted < credit_charge)
	{
		DEBUG(0,
		    ("smb2_validate_message_id: client used more " "credits than granted, mid %llu, charge %llu, " "credits_granted %llu, " "seqnum low/range: %llu/%llu\n", (unsigned long long) message_id, (unsigned long long) credit_charge, (unsigned long long) xconn->smb2.credits.granted, (unsigned long long) xconn->smb2.credits.seq_low, (unsigned long long) xconn->smb2.credits.seq_range));
		return false;
	}
	...
	for (i = 0; i <= (credit_charge - 1); i++)
	{
		...
		DEBUG(11, ("Iterating mid %llu charge %u (sequence %llu)\n", (unsigned long long)message_id, credit_charge, (unsigned long long)id));
...
static void smb2_set_operation_credit(struct smbXsrv_connection *xconn, const struct iovec *in_vector, struct iovec *out_vector)
{
	...
	{
		smbd_server_connection_terminate(xconn, "client error: credit charge > max credits\n");
		return;
	}
	...
	{
		uint16_t additional_possible = xconn->smb2.credits.max - credit_charge;
	...
	DEBUG(10,
	    ("smb2_set_operation_credit: requested %u, charge %u, " "granted %u, current possible/max %u/%u, " "total granted/max/low/range %u/%u/%llu/%u\n", (unsigned int)credits_requested, (unsigned int)credit_charge, (unsigned int)credits_granted, (unsigned int)credits_possible, (unsigned int)current_max_credits, (unsigned int)xconn->smb2.credits.granted, (unsigned int)xconn->smb2.credits.max, (unsigned long long)xconn->smb2.credits.seq_low, (unsigned int)xconn->smb2.credits.seq_range));
}

static void smb2_calculate_credits(const struct smbd_smb2_request *inreq, struct smbd_smb2_request *outreq)
{
	int count, idx;
...
static NTSTATUS smbd_smb2_request_setup_out(struct smbd_smb2_request *req)
{
	...
	for (idx = 1; idx < count; idx += SMBD_SMB2_NUM_IOV_PER_REQ)
	{
		...
		{
			outhdr = talloc_zero_array(mem_ctx, uint8_t, OUTVEC_ALLOC_SIZE);
			if (outhdr == NULL)
			{
				return NT_STATUS_NO_MEMORY;
			}
		...
		SSVAL(outhdr, SMB2_HDR_CREDIT_CHARGE, SVAL(inhdr, SMB2_HDR_CREDIT_CHARGE));
		SIVAL(outhdr, SMB2_HDR_STATUS, NT_STATUS_V(NT_STATUS_INTERNAL_ERROR));
		SSVAL(outhdr, SMB2_HDR_OPCODE, SVAL(inhdr, SMB2_HDR_OPCODE));
		SIVAL(outhdr, SMB2_HDR_FLAGS, IVAL(inhdr, SMB2_HDR_FLAGS) | SMB2_HDR_FLAG_REDIRECT);
		...
		SIVAL(outhdr, SMB2_HDR_TID, IVAL(inhdr, SMB2_HDR_TID));
		SBVAL(outhdr, SMB2_HDR_SESSION_ID, BVAL(inhdr, SMB2_HDR_SESSION_ID));
		memcpy(outhdr + SMB2_HDR_SIGNATURE, inhdr + SMB2_HDR_SIGNATURE, 16);
...
void smbd_server_connection_terminate_ex(struct smbXsrv_connection *xconn, const char *reason, const char *location)
{
	DEBUG(10, ("smbd_server_connection_terminate_ex: reason[%s] at %s\n", reason, location));
	exit_server_cleanly(reason);
}

static bool dup_smb2_vec4(TALLOC_CTX *ctx, struct iovec *outvec, const struct iovec *srcvec)
{
	const uint8_t *srctf;
	size_t srctf_len;
...
static struct smbd_smb2_request *dup_smb2_req(const struct smbd_smb2_request *req)
{
	...
	smb2_setup_nbt_length(newreq->out.vector, newreq->out.vector_count);
...
static NTSTATUS smb2_send_async_interim_response(const struct smbd_smb2_request *req)
{
	...
	smb2_setup_nbt_length(nreq->out.vector, nreq->out.vector_count);
	...
	{
		status = smb2_signing_encrypt_pdu(req->first_key, xconn->protocol, firsttf, nreq->out.vector_count - first_idx);
		if (!NT_STATUS_IS_OK(status))
		{
			return status;
		}
	...
	{
		status = smb2_signing_sign_pdu(req->last_key, xconn->protocol, outhdr_v,
...
static void smbd_smb2_request_pending_timer(struct tevent_context *ev, struct tevent_timer *te, struct timeval current_time, void *private_data);

NTSTATUS smbd_smb2_request_pending_queue(struct smbd_smb2_request *req, struct tevent_req *subreq, uint32_t defer_time)
{
	NTSTATUS status;
	...
	if (req->current_idx > 1)
	{
		...
		if (DEBUGLEVEL >= 10)
		{
			dbgtext("smbd_smb2_request_pending_queue: if (req->current_idx > 1) exit\n");
		}
	}
	if (req->last_key.length > 0)
	{
...
static DATA_BLOB smbd_smb2_signing_key(struct smbXsrv_session *session, struct smbXsrv_connection *xconn)
{
...
static void smbd_smb2_request_pending_timer(struct tevent_context *ev, struct tevent_timer *te, struct timeval current_time, void *private_data)
{
	struct smbd_smb2_request *req = talloc_get_type_abort(private_data, struct smbd_smb2_request);
	struct smbXsrv_connection *xconn = req->xconn;
	...
	if (state == NULL)
	{
		smbd_server_connection_terminate(xconn, nt_errstr(NT_STATUS_NO_MEMORY));
		return;
	}
	...
	SBVAL(hdr, SMB2_HDR_SESSION_ID, BVAL(outhdr, SMB2_HDR_SESSION_ID));
	memcpy(hdr + SMB2_HDR_SIGNATURE, outhdr + SMB2_HDR_SIGNATURE, 16);
	...
	/* Ensure we correctly go through crediting. Grant
	 ...
	 response. */
	smb2_set_operation_credit(req->xconn, SMBD_SMB2_IN_HDR_IOV(req), &state->vector[1 + SMBD_SMB2_HDR_IOV_OFS]);
	...
	if (DEBUGLVL(10))
	{
		...
		{
			dbgtext("\tstate->vector[%u/%u].iov_len = %u\n", (unsigned int) i, (unsigned int) ARRAY_SIZE(state->vector), (unsigned int) state->vector[i].iov_len);
		}
	}
	...
	if (req->do_encryption)
	{
		...
		status = smb2_signing_encrypt_pdu(encryption_key, xconn->protocol, &state->vector[1 + SMBD_SMB2_TF_IOV_OFS],
	...
	else if (req->do_signing)
	{
		...
		status = smb2_signing_sign_pdu(signing_key, xconn->protocol, &state->vector[1 + SMBD_SMB2_HDR_IOV_OFS],
		SMBD_SMB2_NUM_IOV_PER_REQ - 1);
		if (!NT_STATUS_IS_OK(status))
		{
			smbd_server_connection_terminate(xconn, nt_errstr(status));
...
static NTSTATUS smbd_smb2_request_process_cancel(struct smbd_smb2_request *req)
{
	...
	if (cur && cur->subreq)
	{
		...
		DEBUG(10, ("smbd_smb2_request_process_cancel: attempting to " "cancel opcode[%s] mid %llu\n", smb2_opcode_name(SVAL(inhdr, SMB2_HDR_OPCODE)), (unsigned long long)found_id ));
...
static NTSTATUS smbd_smb2_request_check_tcon(struct smbd_smb2_request *req)
{
	...
	status = smb2srv_tcon_lookup(req->session, in_tid, now, &tcon);
	if (!NT_STATUS_IS_OK(status))
	{
		return status;
	}
...
static NTSTATUS smbd_smb2_request_check_session(struct smbd_smb2_request *req)
{
	...
	status = smb2srv_session_lookup(req->xconn, in_session_id, now, &session);
	if (session)
	{
		req->session = session;
	...
	if (in_session_id != req->xconn->client->last_session_id)
	{
		...
		set_current_user_info(session_info->unix_info->sanitized_username, session_info->unix_info->unix_name, session_info->info->domain_name);
	}

	return NT_STATUS_OK;
}

NTSTATUS smbd_smb2_request_verify_creditcharge(struct smbd_smb2_request *req, uint32_t data_length)
{
	struct smbXsrv_connection *xconn = req->xconn;
	...
	DEBUG(10, ("mid %llu, CreditCharge: %d, NeededCharge: %d\n", (unsigned long long) BVAL(inhdr, SMB2_HDR_MESSAGE_ID), credit_charge, needed_charge));
...
NTSTATUS smbd_smb2_request_verify_sizes(struct smbd_smb2_request *req, size_t expected_body_size)
{
...
NTSTATUS smbd_smb2_request_dispatch(struct smbd_smb2_request *req)
{
	...
	if (intf_v->iov_len == SMB2_TF_HDR_SIZE)
	{
		...
		{
			DEBUG(0, ("smbd_smb2_request_dispatch: invalid session_id" "in SMB2_HDR[%llu], SMB2_TF[%llu]\n", (unsigned long long)x->global->session_wire_id, (unsigned long long)tf_session_id));
			/*
	...
	else if (signing_required || (flags & SMB2_HDR_FLAG_SIGNED))
	{
		...
		if (x == NULL)
		{
			...
			if ((opcode == SMB2_OP_NEGPROT) && (flags & SMB2_HDR_FLAG_SIGNED))
			{
		...
		status = smb2_signing_check_pdu(signing_key, xconn->protocol, SMBD_SMB2_IN_HDR_IOV(req),
		SMBD_SMB2_NUM_IOV_PER_REQ - 1);
		if (!NT_STATUS_IS_OK(status))
		{
			return smbd_smb2_request_error(req, status);
...
static NTSTATUS smbd_smb2_request_reply(struct smbd_smb2_request *req)
{
	...
	if (req->do_encryption && (firsttf->iov_len == 0) && (req->first_key.length == 0) && (req->session != NULL) && (req->session->global->encryption_key.length != 0))
	{
		...
		tf = talloc_zero_array(req, uint8_t, SMB2_TF_HDR_SIZE);
		if (tf == NULL)
		{
			return NT_STATUS_NO_MEMORY;
		}
	...
	if ((req->current_idx > SMBD_SMB2_NUM_IOV_PER_REQ) && (req->last_key.length > 0) && (firsttf->iov_len == 0))
	{
		 ...
		 */
		status = smb2_signing_sign_pdu(req->last_key, xconn->protocol, lasthdr,
		SMBD_SMB2_NUM_IOV_PER_REQ - 1);
		if (!NT_STATUS_IS_OK(status))
		{
			return status;
	...
	if (req->current_idx < req->out.vector_count)
	{
		...
		tevent_schedule_immediate(im, req->sconn->ev_ctx, smbd_smb2_request_dispatch_immediate, req);
		return NT_STATUS_OK;
	}
	...
	if (firsttf->iov_len == SMB2_TF_HDR_SIZE)
	{
		status = smb2_signing_encrypt_pdu(req->first_key, xconn->protocol, firsttf, req->out.vector_count - first_idx);
		if (!NT_STATUS_IS_OK(status))
		{
			return status;
		}
	...
	else if (req->do_signing)
	{
		...
		status = smb2_signing_sign_pdu(signing_key, xconn->protocol, outhdr,
		SMBD_SMB2_NUM_IOV_PER_REQ - 1);
		if (!NT_STATUS_IS_OK(status))
		{
			return status;
	...
	if (req->out.vector_count < (2 * SMBD_SMB2_NUM_IOV_PER_REQ) && outdyn->iov_base == NULL && outdyn->iov_len != 0)
	{
...
void smbd_smb2_request_dispatch_immediate(struct tevent_context *ctx, struct tevent_immediate *im, void *private_data)
{
	struct smbd_smb2_request *req = talloc_get_type_abort(private_data, struct smbd_smb2_request);
	struct smbXsrv_connection *xconn = req->xconn;
	NTSTATUS status;
...
NTSTATUS smbd_smb2_request_done_ex(struct smbd_smb2_request *req, NTSTATUS status, DATA_BLOB body, DATA_BLOB *dyn, const char *location)
{
	uint8_t *outhdr;
	...
	DEBUG(10,
	    ("smbd_smb2_request_done_ex: " "idx[%d] status[%s] body[%u] dyn[%s:%u] at %s\n", req->current_idx, nt_errstr(status), (unsigned int)body.length, dyn ? "yes": "no", (unsigned int)(dyn ? dyn->length : 0), location));
	...
	if ((next_command_ofs % 8) != 0)
	{
		...
		if (SMBD_SMB2_OUT_DYN_LEN(req) == 0)
		{
			...
			pad = talloc_zero_array(req, uint8_t, pad_size);
			if (pad == NULL)
			{
		...
		else
		{
			...
			new_dyn = talloc_zero_array(req, uint8_t, new_size);
			if (new_dyn == NULL)
			{
				return smbd_smb2_request_error(req, NT_STATUS_NO_MEMORY);
			}
...
NTSTATUS smbd_smb2_request_error_ex(struct smbd_smb2_request *req, NTSTATUS status, DATA_BLOB *info, const char *location)
{
	struct smbXsrv_connection *xconn = req->xconn;
	DATA_BLOB body;
	DATA_BLOB _dyn;
	...
	if (unread_bytes)
	{
		...
		if (ret != unread_bytes)
		{
			...
			DEBUG(2, ("Failed to drain %u bytes from SMB2 socket: " "ret[%u] errno[%d] => %s\n", (unsigned)unread_bytes, (unsigned)ret, errno, nt_errstr(error)));
			return error;
		}
	}
...
static NTSTATUS smbd_smb2_send_break(struct smbXsrv_connection *xconn, struct smbXsrv_session *session, struct smbXsrv_tcon *tcon, const uint8_t *body, size_t body_len)
{
	...
	statelen = offsetof(struct smbd_smb2_send_break_state, body) + body_len;
	...
	state->vector[0] = (struct iovec
			...
			{ .iov_base = state->nbt_hdr, .iov_len = sizeof(state->nbt_hdr) };

	if (do_encryption)
	{
		state->vector[1 + SMBD_SMB2_TF_IOV_OFS] = (struct iovec
				)
				{ .iov_base = state->tf, .iov_len = sizeof(state->tf) };
	}
	else
	{
		state->vector[1 + SMBD_SMB2_TF_IOV_OFS] = (struct iovec
	...
	state->vector[1 + SMBD_SMB2_BODY_IOV_OFS] = (struct iovec
			...
			{ .iov_base = state->body, .iov_len = body_len /* no sizeof(state->body) .. :-) */
			};
...
NTSTATUS smbd_smb2_send_oplock_break(struct smbXsrv_connection *xconn, struct smbXsrv_session *session, struct smbXsrv_tcon *tcon, struct smbXsrv_open *op, uint8_t oplock_level)
{
	uint8_t body[0x18];
...
NTSTATUS smbd_smb2_send_lease_break(struct smbXsrv_connection *xconn, uint16_t new_epoch, uint32_t lease_flags, struct smb2_lease_key *lease_key, uint32_t current_lease_state,
...
static bool is_smb2_recvfile_write(struct smbd_smb2_request_read_state *state)
{
	...
	status = smb2srv_open_lookup(state->req->xconn, file_id_persistent, file_id_volatile, 0, /* now */
	&op);
	if (!NT_STATUS_IS_OK(status))
	{
		return false;
...
void smbd_smb2_first_negprot(struct smbXsrv_connection *xconn, const uint8_t *inpdu, size_t size)
{
	struct smbd_server_connection *sconn = xconn->client->sconn;
	NTSTATUS status;
...
static int socket_error_from_errno(int ret, int sys_errno,
bool *retry)
{
	*retry = false;
...
static NTSTATUS smbd_smb2_flush_send_queue(struct smbXsrv_connection *xconn)
{
	...
	while (xconn->smb2.send_queue != NULL)
	{
		...
		if (e->sendfile_header != NULL)
		{
			...
			for (i = 0; i < e->count; i++)
			{
				memcpy(buf + size, e->vector[i].iov_base, e->vector[i].iov_len);
				size += e->vector[i].iov_len;
			}
...
static NTSTATUS smbd_smb2_io_handler(struct smbXsrv_connection *xconn, uint16_t fde_flags)
{
	struct smbd_server_connection *sconn = xconn->client->sconn;
	struct smbd_smb2_request_read_state *state = &xconn->smb2.request_read_state;
	struct smbd_smb2_request *req = NULL;
	...
	if (state->pktlen > 0)
	{
		if (state->doing_receivefile && !is_smb2_recvfile_write(state))
		{
			...
			state->pktbuf = talloc_realloc(state->req, state->pktbuf, uint8_t, state->pktfull);
			if (state->pktbuf == NULL)
			{
				return NT_STATUS_NO_MEMORY;
			}

			state->vector.iov_base = (void *) (state->pktbuf + state->pktlen);
			state->vector.iov_len = (state->pktfull - state->pktlen);
	...
	status = smbd_smb2_inbuf_parse_compound(xconn, now, state->pktbuf, state->pktlen, req, &req->in.vector, &req->in.vector_count);
	if (!NT_STATUS_IS_OK(status))
	{
		return status;
	}
	...
	if ((sconn->num_requests % 50) == 0 && need_to_check_log_size())
	{
		change_to_root_user();
		check_log_size();
	}
...
static void smbd_smb2_connection_handler(struct tevent_context *ev, struct tevent_fd *fde, uint16_t flags, void *private_data)
{
	struct smbXsrv_connection *xconn = talloc_get_type_abort(private_data, struct smbXsrv_connection);
	NTSTATUS status;
